...
const AIChatbot = () => {
        ...
        if (isOpen && !chatSessionRef.current) {
            ...
            try {
                ...
                chatSessionRef.current = ai.chats.create({
                    ...
                    config: {
                        systemInstruction: `You are "personal assistant", Raghav Maheshwari's Portfolio Assistant. Be concise, professional, and witty. Knowledge: ${JSON.stringify(contextData)}.`,
                    }
                });

                setMessages([{
                    role: 'model',
                    text: "Online. I am personal assistant. Ask me about Raghav's projects or skills."
                }]);
            } catch (err) {
    ...
    const handleSend = async () => {
        ...
        if (!chatSessionRef.current) {
            setMessages(prev => [...prev, { role: 'model', text: "Error: Chat not active." }]);
            return;
        }
                    ...
                    <div className="neon-inner">
                        ...
                        <div className="p-4 border-b border-white/10 flex items-center justify-between shrink-0 bg-[#0f0f0f]">
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    <h3 className="font-bold text-sm text-white tracking-wide">ChatBot</h3>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-gray-800 scrollbar-track-transparent">
                            {messages.map((msg, idx) => (
                                <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                    <div className={`max-w-[85%] px-3 py-2.5 rounded-2xl text-xs sm:text-sm leading-relaxed shadow-sm ${msg.role === 'user'
